...
func (n *Notification) BeforeCreate(tx *gorm.DB) error {
	...
	if len(n.Recipients) > 0 {
		...
		if len(recipients) == 0 {
			return fmt.Errorf("recipients cannot be an empty array")
		}
	}
	return nil
}
...
func (n *Notification) MarkAsSent() error {
	if n.Status != NotificationStatusProcessing {
		return fmt.Errorf("cannot mark as sent: current status is %s", n.Status)
	}
	now := time.Now()
...
func (n *Notification) MarkAsRead() error {
	if n.Status != NotificationStatusSent {
		return fmt.Errorf("cannot mark as read: current status is %s", n.Status)
	}
...
// MulticastResult holds per-token results from a multicast push send.
type MulticastResult struct {
	SuccessCount int
	FailedTokens []string // tokens that permanently failed (e.g. unregistered)
}

...
// Start starts the cleanup routine. Must be called before Register.
...
func (cm *ConnectionManager) cleanupIdleConnections() {
	...
	for clientID, client := range cm.clients {
		...
		if client.IsIdle(cm.config.IdleTimeout) {
			...
			idleCount++
		} else if client.IsClosed() || !client.IsReady() {
			// Also remove closed or not-ready (zombie) connections
			toRemove = append(toRemove, clientID)
		}
	}
...
func (cm *ConnectionManager) Shutdown(ctx context.Context) error {
	...
	// Signal shutdown
	cm.shutdownOnce.Do(func() { close(cm.shutdownCh) })
	cm.cancel()

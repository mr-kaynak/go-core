...
// Start starts the worker pool and batch processor. Must be called before broadcasting.
...
func (eb *EventBroadcaster) broadcast(
	...
	// Increment queued jobs counter (decremented in processJob when dequeued)
...
func (eb *EventBroadcaster) processJob(job *BroadcastJob) {
	// Decrement queued counter now that the job is dequeued
	atomic.AddInt32(&eb.queuedJobs, -1)
	...
	// Send to clients and log results
	eb.sendToClients(ctx, job, clients)
}

// sendToClients sends the event to the given clients and logs results
func (eb *EventBroadcaster) sendToClients(ctx context.Context, job *BroadcastJob, clients []*streaming.Client) {
	successCount := 0
	failCount := 0
...
// handleSendError handles errors from sending an event to a client
func (eb *EventBroadcaster) handleSendError(err error, job *BroadcastJob) {
	if err == streaming.ErrClientClosed {
		return
	}
...
func (eb *EventBroadcaster) Shutdown(ctx context.Context) error {
	...
	// Signal shutdown (idempotent via sync.Once — P0-002)
	eb.shutdownOnce.Do(func() { close(eb.shutdownCh) })

	// Process remaining jobs before canceling context
	...
	// Cancel context after drain is complete (or timed out)
	eb.cancel()

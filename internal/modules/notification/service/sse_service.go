...
func (s *SSEService) Start() error {
	...
	// Start core components — goroutines are deferred to Start() to avoid
	// leaking goroutines if SSEService.Start() is never called (P1-007).
	s.connManager.Start()
	s.broadcaster.Start()
	...
	if err := s.heartbeat.Start(); err != nil {
		s.cancel()
		_ = s.connManager.Shutdown(context.Background())
		_ = s.broadcaster.Shutdown(context.Background())

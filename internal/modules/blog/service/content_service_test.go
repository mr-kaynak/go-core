...
func TestContentService_SerializeToHTML_Image(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "url field fallback",
			input: `[{"type":"img","url":"/api/v1/blog/media/file/blog/abc/photo.png","children":[{"text":""}]}]`,
			want:  `<img src="/api/v1/blog/media/file/blog/abc/photo.png" alt="" />`,
		},
	}
...
func TestContentService_EndToEnd_ImageWithRelativeURL(t *testing.T) {
	svc := NewContentService()
	...
	if err != nil {
		t.Fatalf("SerializeToHTML error: %v", err)
	}
	t.Logf("SerializeToHTML output: %q", html)

	if !strings.Contains(html, `src="/api/v1/blog/media/file/blog/1b6a2a1e`) {
		t.Errorf("SerializeToHTML missing src, got: %q", html)
	}

	// Step 2: SanitizeHTML (this is what happens in PostService)
	sanitized := svc.SanitizeHTML(html)
	t.Logf("SanitizeHTML output: %q", sanitized)

	if !strings.Contains(sanitized, `/api/v1/blog/media/file/blog/1b6a2a1e`) {
		t.Errorf("SanitizeHTML stripped relative img src, got: %q", sanitized)
	}
}

func TestContentService_SanitizeHTML_RelativeImgSrcPreserved(t *testing.T) {
	svc := NewContentService()

	input := `<img src="/api/v1/blog/media/file/blog/abc/photo.png" alt="test" />`
	got := svc.SanitizeHTML(input)

	if !strings.Contains(got, `/api/v1/blog/media/file/blog/abc/photo.png`) {
		t.Errorf("SanitizeHTML stripped relative img src, got: %q", got)
	}
}

...
func (s *EngagementService) ToggleLike(ctx context.Context, postID, userID uuid.UUID) (*ToggleLikeResponse, error) {
	var liked bool
	var likeCount int
	...
	if txErr != nil {
		return nil, errors.NewInternalError("Failed to toggle like")
	}
	...
	if s.sseSvc != nil {
		...
		_ = s.sseSvc.BroadcastToChannel(ctx, "blog:engagement", event)
	}
...
func (s *EngagementService) RecordView(ctx context.Context, postID uuid.UUID, userID *uuid.UUID, ip, userAgent, referrer string) error {
	...
	if s.redisClient != nil {
		...
		acquired, err := s.redisClient.SetNX(ctx, cacheKey, "1", cooldown)
		if err == nil && !acquired {

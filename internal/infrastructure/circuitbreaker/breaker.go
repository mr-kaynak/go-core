...
type CircuitBreaker struct {
	...
	mu                  sync.RWMutex
	lastStateChangeNano int64 // atomic, unix nano
}
...
func New(config Config) *CircuitBreaker {
	cb := &CircuitBreaker{
		...
		lastStateChangeNano: time.Now().UnixNano(),
	}
	return cb
}
...
func (cb *CircuitBreaker) RecordSuccess() {
	...
	// If in half-open state, track successful completions separately
	if state == StateHalfOpen {
		successes := atomic.AddInt32(&cb.halfOpenSuccesses, 1)
		if successes >= int32(cb.config.HalfOpenMaxRequests) {
...
func (cb *CircuitBreaker) GetStats() Stats {
	...
	return Stats{
		...
		LastStateChange:     time.Unix(0, atomic.LoadInt64(&cb.lastStateChangeNano)),
	}
}
...
// lastStateTime is protected by storing as atomic int64.
func (cb *CircuitBreaker) onStateChange(from, to State) {
	atomic.StoreInt64(&cb.lastStateChangeNano, time.Now().UnixNano())
...
func (sw *slidingWindow) Record(success bool) {
	...
	// Advance through all elapsed buckets, clearing stale data
	elapsed := now.Sub(sw.buckets[sw.current].timestamp)

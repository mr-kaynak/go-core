...
func Initialize(level, format, output string) error {
	...
	case "stderr":
		writer = os.Stderr
	default:
		file, err := os.OpenFile(output, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o600) //nolint:mnd // owner-only file permission
		if err != nil {
...
// Fatal logs a fatal message, flushes output, and exits
...
// sensitiveKeys contains field names that should be redacted in log output.
...
// isSensitiveKey checks if a log field key should be redacted.
